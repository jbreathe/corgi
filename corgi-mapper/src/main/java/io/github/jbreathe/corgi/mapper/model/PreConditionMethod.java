...
/**
 * Representation of method annotated with {@link io.github.jbreathe.corgi.api.PreCondition}.
 */
public final class PreConditionMethod {
    private static final String PARAMETER_NOT_FOUND = "Parameter '%s' of @PreCondition method '%s' must be declared in the associated @Mapping method '%s'";
    ...
    public PreConditionMethod(Type consumerType, String name, TypeDeclaration resultType, List<VarDeclaration> parameters) {
    ...
    public BooleanExpression generateCall(MappingMethod mappingMethod, MethodCall readCall, Field field) {
        List<Expression> arguments = collectArguments(mappingMethod, readCall, field);
        MethodCall methodCall = MethodCall.callWithArgs(name, resultType, thisReference, arguments);
        return BooleanExpression.fromMethodCall(methodCall);
    }
